...
func (process *TeleportProcess) getAdditionalPrincipals(role types.SystemRole) ([]string, []string, error) {
	...
	// Add default principals list to the addrs list.
	// For identities generated by teleport <= v6.1.6 the teleport.cluster.local principal is not present
	for _, v := range auth.DefaultPrincipalsForRole(role) {
		addrs = append(addrs, utils.NetAddr{Addr: v})
	}
